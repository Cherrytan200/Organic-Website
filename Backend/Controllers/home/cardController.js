...
// Get cart products
export const get_card_products = async (req, res) => {
    const co = 5;
        ...
        const card_products = await cardModel.aggregate([
            {
                $match: {
                    userId: {
                        $eq: new ObjectId(userId), // Correct usage of ObjectId
                    },
                },
            },
            {
...
export const delete_card_products = async (req, res) => {
...
// Increase quantity
export const quantity_inc = async (req, res) => {
    const { card_id } = req.params;
    try {
...
// Decrease quantity
export const quantity_dec = async (req, res) => {
    const { card_id } = req.params;
    try {
        const product = await cardModel.findById(card_id);
        const { quantity } = product;
